...
description: Import Sumble's MCP, generate a workflow-grouped plugin, validate outputs for Claude Code, Cursor, Codex, and OpenCode, and review the current Codex hook activation caveat.
---
...
still be guarded by hooks = true under [features] in the host runtime.
Older docs or configs may still refer to codex_hooks. If bundled hooks do not
activate, enable the current feature flag and reload Codex.
```

What this means:

- Claude Code, Cursor, and Codex all receive packaged hook output from this scaffold
- Codex may still require `[features].hooks = true` in project or user config before those bundled hooks activate
