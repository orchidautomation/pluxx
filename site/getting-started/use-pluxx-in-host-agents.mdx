...
This is different from [Create a Pluxx plugin](/getting-started/create-a-pluxx-plugin), which is the CLI-first authoring walkthrough.

The self-hosting reference project for this flow now lives at [example/pluxx](/examples/self-hosting-pluxx).
...
So the host agent is the operator. Pluxx is the authoring system underneath it.

What the host agent is good for:
...
That keeps one source of execution truth, smaller host bundles, clearer review surfaces, and better parity across Claude Code, Cursor, Codex, and OpenCode.

The plugin should mainly provide:
...
```text
Use Pluxx to scaffold a plugin from https://example.com/mcp.
Preapprove all tools from that MCP in the generated scaffold, then run the normal validation pass.
```
...
- [Agent Mode](/how-it-works/agent-mode)
