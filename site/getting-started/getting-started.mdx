...
- [Create a Pluxx plugin](/getting-started/create-a-pluxx-plugin)
- [Use Pluxx in host agents](/getting-started/use-pluxx-in-host-agents)
- [Practical handbook](/getting-started/practical-handbook)
- [How it works](/how-it-works/how-it-works)
- [Architecture](/how-it-works/architecture)
- [Customer journey](/workflows/import-from-mcp)
- [One-shot autopilot spec](/workflows/autopilot)
- [Sumble proof run](/examples/sumble-proof-run)
- [Homepage messaging and narrative](https://github.com/orchidautomation/pluxx/blob/main/docs/homepage-messaging.md)
- [PlayKit case study](/examples/firecrawl-sumble-playkit)
...
## 6. Extra Commands Worth Knowing

These are now part of the real baseline, not side experiments:

- `npx @orchid-labs/pluxx migrate <existing-plugin-path>`
  - import an existing host-native plugin into a Pluxx source project
- `npx @orchid-labs/pluxx doctor --consumer <built-or-installed-bundle>`
  - inspect the plugin from the consumer side after installation
- `npx @orchid-labs/pluxx mcp proxy --from-mcp https://example.com/mcp --record tape.json`
  - capture a deterministic MCP tape for debugging or CI
- `npx @orchid-labs/pluxx mcp proxy --replay tape.json`
  - replay the same MCP session without hitting the live server again

## 7. See A Real MCP Proof

If you want to see the full loop against a live commercial MCP, read the [Sumble proof run](/examples/sumble-proof-run).

It covers:

- workflow-grouped import from a real remote MCP
- core-four build and test output
- the Codex headless runner fix discovered during the run
