...
Once Pluxx is published, the launcher in `bin/pluxx.js` is already set up so `npx pluxx ...` can work on machines that also have Bun installed. But the honest current docs contract is Bun-first and repo/local-package-first.
