...
OpenCode wants:    package.json + host entry code + JS/runtime config surfaces
```

Each platform also has its own validation and packaging rules. Some are published hard requirements, and some limits Pluxx treats conservatively as heuristics so generated bundles still land in native-looking territory. Discovering those differences by hand is trial and error.
...
Pluxx also gets materially better when you feed it real product context, not just an MCP endpoint.

Today, the main place to do that is:

- `pluxx agent prepare --website ... --docs ...`

That gives the host runner better raw material for:
...
The current product direction is to make docs and website ingestion more first-class over time, especially around `init`, `autopilot`, and `agent prepare`.
...
- keep the CLI as the execution engine
- keep the Pluxx plugin thin and native to each host

That avoids shipping a second runtime contract inside every plugin bundle.
...
The generated source of truth for support status and verification coverage is [docs/compatibility.md](/how-it-works/compatibility-limits).
