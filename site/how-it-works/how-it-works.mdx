...
The launcher in `bin/pluxx.js` delegates to Bun when it is not already running under Bun, so the honest runtime contract is: npm distribution is real, but Bun must still be installed on the machine.
