...
- generated installers prepare native Node dependencies in a Pluxx-managed shared runtime store when the bundle has deterministic dependency metadata; the store is keyed by the complete installed bundle payload that can affect runtime installation, OS, architecture, Node ABI, and runtime contract version
