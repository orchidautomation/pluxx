...
- generated installers prepare native Node dependencies in a Pluxx-managed shared runtime store when the bundle has a deterministic dependency manifest; the store is keyed by dependency shape, lockfile content, bootstrap script, OS, architecture, Node ABI, and runtime contract version
