...
- generated installers prepare native Node dependencies in a Pluxx-managed shared runtime store when the bundle has deterministic dependency metadata; the store is keyed by deterministic installed bundle files that can affect runtime installation, OS, architecture, Node ABI, and runtime contract version, excluding host manifests, materialized MCP config, saved user config, and install-local state
