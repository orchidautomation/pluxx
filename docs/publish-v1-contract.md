...
- generated installers prepare native Node dependencies in a Pluxx-managed shared runtime store when the bundle has deterministic dependency metadata, including a supported lockfile; the store is keyed by deterministic installed bundle files that can affect runtime installation, OS, architecture, Node ABI, and runtime contract version, excluding host manifests, materialized MCP config, saved user config, and install-local state
...
- when a bundle does not expose enough dependency metadata for safe shared-runtime reuse, including a supported deterministic lockfile, generated installers log the fallback and keep the previous per-host staged bootstrap behavior
