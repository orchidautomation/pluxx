...
- plugin-bundled hook activation still needs the canonical `[features].hooks = true` feature key plus enabled plugin state, user review, project trust, and runtime support. The deprecated `[features].codex_hooks` alias still appears in local runtime behavior, and current `codex exec --help` exposes `--enable <FEATURE>` for the CLI path, but maintained local probes on June 24, 2026 still failed to execute the project-local hook under the canonical flag, deprecated alias, or `--enable hooks`.
...
- current live probes now also show a sharper Codex MCP caveat: project-scoped and user-scoped root MCP config reached `initialize`, `notifications/initialized`, and `tools/list` in headless `codex exec`, but default root-scoped paths were still auto-cancelled with `user cancelled MCP tool call` before any server-side `tools/call`. The maintained June 24 suite proves approved project-root and user-root MCP, plus custom agents inheriting those approved root servers, including explicit child-agent `mcp_servers = {}` override cases. An August 31 refresh on Codex CLI 0.148.0 showed inline MCP remaining unavailable without approval while explicit per-tool approval reached server-side `tools/call`, closing the approved-inline gap. The approved delegated paths now surface a parent `mcp_tool_call` item; project-scoped servers still do not appear in `codex mcp list`, and user-scoped servers do appear there. Pluxx already uses the root-config proof by generating `.codex/config.generated.toml` when canonical top-level MCP allow rules can be materialized safely
