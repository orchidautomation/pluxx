...
- generated release installers use that split to prepare one content-addressed native Node runtime under `~/.pluxx/runtimes/` when package dependency metadata and a supported deterministic lockfile are available, then link each compatible staged host bundle to the same immutable `node_modules`
