...
- Related:
  ...
  - [docs/orchid/plans/2026-07-14-pluxx-328-native-adjuncts.md](./orchid/plans/2026-07-14-pluxx-328-native-adjuncts.md)
  - [docs/orchid/decisions/2026-07-14-pluxx-328-native-adjuncts.md](./orchid/decisions/2026-07-14-pluxx-328-native-adjuncts.md)
  - [docs/orchid/plans/2026-07-14-pluxx-329-core-four-release-gate.md](./orchid/plans/2026-07-14-pluxx-329-core-four-release-gate.md)
  - [docs/orchid/decisions/2026-07-14-pluxx-329-core-four-release-gate.md](./orchid/decisions/2026-07-14-pluxx-329-core-four-release-gate.md)
  - [docs/orchid/reviews/2026-07-14-pluxx-329-review.md](./orchid/reviews/2026-07-14-pluxx-329-review.md)
...
If you want the current pressure test for skill-owned subagent orchestration, mature multi-host import, and installed/runtime proof across the frozen core four—Claude Code, Cursor, Codex, and OpenCode—use the [Compound Engineering primitive audit](./orchid/requirements/2026-07-13-compound-engineering-primitive-audit.md), [three-reference comparison](./orchid/requirements/2026-07-14-orchestration-reference-patterns.md), [accepted orchestration decision](./orchid/decisions/2026-07-14-orchestration-primitive.md), [phased parity plan](./orchid/plans/2026-07-13-compound-engineering-parity-plan.md), and the Phase 0–6 implementation plans ending with [PLUXX-329](./orchid/plans/2026-07-14-pluxx-329-core-four-release-gate.md). Mature migration recovers supported native adjuncts with exact provenance and transactional refusal semantics. One hermetic `npm run core-four:proof` gate now revalidates the 44-row pinned inventory, 12 receipts, 176 adjunct outcomes, current registries and maintained compatibility output, two deterministic 12-case replays, and clean symlink-root plus copied-install ownership preimages before release packaging. Generated registration artifacts remain isolated evidence only: real-host discovery is environment-unavailable in all 12, activation is unsupported, behavioral parity is unproved, and all 324 orchestration outcomes remain degraded. Phase 4 secondary-target expansion stays deferred; Phase 7 freezes the initiative to the core four. The Phase 0–6 stack is ready for PR packaging, not a runtime-parity claim.
...
- runtime readiness is now a first-class runtime primitive:
  ...
  - managed Claude settings proof is intentionally bounded here: the opt-in managed-shadow `SessionStart` scenarios run through a synthetic managed-settings path in the unit harness; the June 24 live Claude run did not treat that synthetic path as real managed settings, so registry/plist/MDM/server-managed precedence and managed-scope plugin behavior remain unproven in this environment
  - `bun scripts/probe-codex-hooks-runtime.ts --json` now provides a maintained isolated headless Codex hook probe; on 2026-06-24 it showed `hooks-no-trust`, `hooks-trusted`, `codex-hooks-trusted`, and optional `enable-hooks-trusted` all completing with no hook side effect
  ...
  - the optional `--include-enable-hooks-cli` Codex hook scenarios now show that the current CLI feature path does not rescue activation either: the June 24 headless `enable-hooks-trusted` still completed with no hook side effect, and the earlier trusted interactive `user-prompt-submit-enable-hooks-trusted` plus `session-start-enable-hooks-trusted` still timed out with no hook side effect and no `/hooks` review gate
