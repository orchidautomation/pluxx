...
| Hook `matcher` field | `preserve -> hook entries` | `preserve -> hook entries` | `translate -> matcher survives on documented Codex matcher-capable events in bundled hook groups and .codex/hooks.generated.json` | `translate -> string matchers and object matchers with a string tool field become runtime tool filters` | Matching logic is portable enough to keep, even when the storage layer changes. OpenCode separates explicitly unscoped all-tool hooks from matcher-scoped hooks, maps edit intent across `edit`, `write`, and `apply_patch`, and fails closed for richer object matcher shapes it cannot translate exactly. |
...
- [New-host support gate](./new-host-support-gate.md)
