...
| Prompt-based hooks | `degrade -> host can support more, but current generator drops prompt hooks with warnings` | `preserve -> hooks/hooks.json prompt entries` | `translate/degrade -> beforeSubmitPrompt becomes UserPromptSubmit developer-context hooks in .codex/hooks.generated.json; other prompt-hook events still drop` | `drop -> current runtime wrapper only emits command hooks today` | Prompt-driven hook automation is becoming more portable, but Codex still only carries the UserPromptSubmit slice truthfully today. |
...
|---|---|---|---|---|---|
| Canonical `allow` / `ask` / `deny` DSL | `translate -> runtime approvals, hook decisions, and agent surfaces` | `translate -> generated permission hook plus CLI/subagent control planes` | `translate -> approvals, sandbox policy, hook matchers, custom agents, and .codex/permissions.generated.json guidance for approval_policy + sandbox_mode` | `preserve -> config-native permission map plus per-agent overrides` | The user-facing permission story stays coherent, but only OpenCode has a closer direct config analog today. |
