...
- Codex: preserve plugin-bundled hook intent through `hooks/hooks.json`, while still acknowledging project/user hook config and the `codex_hooks` runtime flag
...
|---|---|---|---|---|---|
| Command-based lifecycle hooks on common events | `preserve -> hooks/hooks.json, manifest hooks, settings hooks, frontmatter hooks` | `preserve -> hooks/hooks.json plus project/user hook files` | `preserve -> hooks/hooks.json plus project/user hook files` | `translate -> JS/TS runtime event handlers` | Hook automation is real across the four. Codex now supports bundled lifecycle config, while OpenCode remains code-first. |
...
| Hook `matcher` field | `preserve -> hook entries` | `preserve -> hook entries` | `preserve -> hook matcher groups` | `translate -> matcher survives in runtime hook definitions` | Matching logic is portable enough to keep, even when the storage layer changes. |
| Hook `failClosed` | `degrade -> current Claude-family generator drops it` | `preserve -> hook entries` | `preserve -> command hook entries` | `translate -> runtime hook definitions` | Strict failure behavior is not yet consistent in Claude outputs. |
...
| Event fidelity | `preserve -> richest event spread of the four` | `preserve -> documented Cursor hook events` | `degrade -> only the documented Codex subset is emitted today, including PermissionRequest` | `translate -> canonical events mapped into runtime event names` | Hook intent is portable, but event names and supported event counts are not. |
...
| Runtime readiness dependency, session-start refresh, and gate polling | `translate -> generated SessionStart / PreToolUse / UserPromptSubmit hooks plus helper runtime script` | `translate -> generated sessionStart / beforeMCPExecution / beforeSubmitPrompt hooks plus helper runtime script` | `degrade -> hooks/hooks.json plus .codex/readiness.generated.json, still gated by codex_hooks and best-effort prompt-entry scoping` | `translate -> runtime/pluxx-readiness.mjs plus event-handler calls from plugin code` | Runtime readiness is operational intent, not manual per-host hook glue. Codex now bundles the lifecycle config, but runtime support still depends on the feature flag and best-effort named prompt targeting. |
