...
| Hook `matcher` field | `preserve -> hook entries` | `preserve -> hook entries` | `translate -> matcher survives on documented Codex matcher-capable events in bundled hook groups and .codex/hooks.generated.json` | `translate -> string matchers become anchored runtime filters; recognized edit-only alternatives route only through edit, write, and apply_patch` | Matching logic is portable enough to keep, even when the storage layer changes. OpenCode has no exact equivalent for structured matcher records, so Pluxx skips those hooks rather than widening them and surfaces the degradation through consumer doctor checks. |
