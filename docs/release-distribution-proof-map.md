...
Proof state is governed by [proof-freshness.md](./proof-freshness.md) and the machine-readable [proof manifest](./proof-manifest.json). The canonical repository version is `0.1.36`, prepared for tag `v0.1.36`. The previous published `v0.1.35` release is historical; older host runs linked below are historical unless a current receipt says otherwise. PLUXX-337 is the active release-prep slice for installed OpenCode top-level wrappers preserving the selected workspace context.
