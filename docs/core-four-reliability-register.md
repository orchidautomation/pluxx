...
- the newest live Codex runtime evidence is now more specific:
  ...
  - the August 31 Codex CLI 0.148.0 refresh preserves the unapproved `agent-inline` gap but proves `agent-inline-approve`: the inline server reaches `initialize`, `notifications/initialized`, `tools/list`, and server-side `tools/call`, returning `MCP_PROOF_MARKER_ALLOWED` when explicit per-tool approval is present
...
- Strong proof:
  ...
  - the August 31 refresh sharpens that result: the default project-root path now reports `MCP tool call requires approval, but approval policy is never`; approved project-root, inherited-root, empty-child-override, and agent-inline paths reach real server-side `tools/call`; unapproved `agent-inline` still returns `MCP_PROOF_MARKER_MISSING`
...
1. deepen Claude and Codex live proof for the remaining highest-risk edges:
   ...
   - deeper Codex custom-agent config-depth proof beyond the now-pinned headless-plus-interactive `sandbox_mode = "read-only"` mismatch, the newly pinned headless `skills.config` caveats, and the newly pinned invalid agent-local model failure path, especially installed-plugin skill preload, unapproved inline MCP behavior, other approval or sandbox combinations, plus the remaining Codex and Claude adjunct distribution-surface proof
