...
Current reviewed receipts for v0.1.35 are `v0.1.35-repository-validation` (`bundle-contract`, `current`) and `v0.1.35-fake-home-install` (`fake-home-install`, `current`). They cite committed OpenCode plugin-root release-prep state before the tag is pushed. The v0.1.34, v0.1.33, v0.1.32, and older receipts remain historical. Neither current tier is installed-runtime or real-host behavior evidence.
