...
| Local stdio under active dev | `npx -y -p @acme/mcp acme-mcp` with changing tools | Can drift quickly as local command or schema changes | Generated taxonomy can go stale after server changes | `pluxx sync --from-mcp` + metadata ownership keeps managed files refreshable |
