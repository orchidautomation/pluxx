...
export function buildHookCommandWrapperScript(command: string, pluginRootVar: string, envFileVar?: string): string {
  ...
  return [
    ...
    'const wrapperBundleRoot = resolve(dirname(scriptPath), "..")',
    'const configuredPluginRoot = process.env[PLUGIN_ROOT_VAR]',
    'const pluginRoot = configuredPluginRoot && resolve(configuredPluginRoot) === wrapperBundleRoot',
    '  ? resolve(configuredPluginRoot)',
    '  : wrapperBundleRoot',
    ...
    '  if (payloadWorkspace) {',
    '    process.env.PLUXX_HOOK_WORKSPACE_ROOT = payloadWorkspace',
    '    return',
    '  }',
    '',
    '  const envCandidates = [',
