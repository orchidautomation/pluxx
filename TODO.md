...
- [ ] `npm login` + `npm publish` so `npx plugahh init` works for real
- [ ] Update Codex CLI (`npm install -g @openai/codex`) and run review
- [ ] Custom domain for landing page (plugahh.dev)
...
- [ ] `plugahh test` — dry-run skills, validate MCP connections
...
## Business — Paid Tier (plugahh.dev)

- [ ] Landing page on custom domain (plugahh.dev)
- [ ] Waitlist for cloud features
- [ ] Analytics dashboard MVP (installs per platform per day)
- [ ] `plugahh publish` cloud service (one-click to all marketplaces)
...
| 3 | "Building an MCP plugin in 5 minutes with plugahh" tutorial | Blog, YouTube |
| 4 | "The Agent Skills standard explained" | Blog |
| 5 | "Why every SaaS needs AI agent plugins" (thought leadership) | Blog, LinkedIn |

## Research / Open Questions

- [ ] Should we rename before launch? (plugahh vs plugforge vs plugsmith — all available on npm)
- [ ] Vercel or Cloudflare Pages for the landing page?
- [ ] Mintlify for docs? (Ultracite uses it)
- [ ] Should `plugahh migrate` support importing from Cursor marketplace plugins?
...
- [x] GitHub repo public at orchidautomation/plugahh
